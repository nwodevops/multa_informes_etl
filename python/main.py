#!/usr/bin/env python3
"""ENTRY POINT de la capa de lógica (orquestación delgada).

Flujo:
  1. SETUP   : root + variables de project-config.json
  2. ENTRADA : io/leer_h2.py              -> DataFrames (nombres = claves de LECTURAS)
  3. LOGICA  : el UNICO .py de python/logica/  -> deja RESULTADO
  4. SALIDA  : Excel (siempre) + MySQL/Oracle (skip si placeholders)

ZONA DE PEGADO: para un ETL nuevo solo se copia un .py a python/logica/ y se deja
un DataFrame RESULTADO. El I/O (python/io/) no se toca.
Contrato: python/CONTRATO.md
Uso: .venv/bin/python python/main.py
"""

from __future__ import annotations

import importlib.util
import sys
from pathlib import Path

HERE = Path(__file__).resolve().parent
if str(HERE) not in sys.path:
    sys.path.insert(0, str(HERE))

from config import load_vars, project_root  # noqa: E402

SALIDA_DF = "RESULTADO"


def _load(name: str, path: Path):
    spec = importlib.util.spec_from_file_location(name, path)
    if spec is None or spec.loader is None:
        raise ImportError(f"No se pudo cargar {path}")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def main() -> int:
    root = project_root()
    variables = load_vars(root)

    leer = _load("leer_h2", HERE / "io" / "leer_h2.py")
    datos = leer.leer_h2(root, variables)
    if not datos:
        raise SystemExit("leer_h2() no devolvió DataFrames; revisa LECTURAS en python/io/leer_h2.py")

    logica_dir = HERE / "logica"
    archivos = sorted(p for p in logica_dir.glob("*.py") if p.name != "__init__.py")
    if not archivos:
        raise SystemExit(
            "No hay ningun .py en python/logica/. Pega ahi tu logica (ver python/plantilla_logica.py)"
        )
    if len(archivos) > 1:
        raise SystemExit(
            f"Hay mas de un .py en python/logica/: {len(archivos)}. Deja un solo archivo de logica."
        )

    print(f"Logica: {archivos[0].name}")
    import pandas as pd

    ns: dict = {"__name__": "__logica__", "pd": pd}
    ns.update(datos)
    exec(compile(archivos[0].read_text(encoding="utf-8"), str(archivos[0]), "exec"), ns)

    if SALIDA_DF not in ns:
        raise SystemExit(
            f"La logica no dejo el DataFrame '{SALIDA_DF}' (configurable en main.py). Ver python/CONTRATO.md"
        )
    df_salida = ns[SALIDA_DF]
    print(f"Salida de logica: {len(df_salida)} filas x {len(df_salida.columns)} columnas")

    excel = _load("escribir_excel", HERE / "io" / "escribir_excel.py")
    excel.escribir_excel(df_salida, root)

    mysql = _load("escribir_mysql", HERE / "io" / "escribir_mysql.py")
    mysql.escribir_mysql(df_salida, root)

    oracle = _load("escribir_oracle", HERE / "io" / "escribir_oracle.py")
    oracle.escribir_oracle(df_salida, root)

    print("Listo (H2 -> logica -> destino).")
    return 0


if __name__ == "__main__":
    try:
        raise SystemExit(main())
    except (FileNotFoundError, ValueError, KeyError) as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
