...
Flujo:
  inputs.yaml → introspect (oracle|sheets|excel|mysql) → CREATE TABLE STG_* en H2
...
def main(argv: list[str] | None = None) -> int:
    ...
    for src in sources:
        ...
        if typ == "mysql" and not src.get("sql_file"):
            require_live_conn(src.get("connection") or "mysql", variables)
