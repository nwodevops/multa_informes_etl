...
El modelo dimensional y los indicadores K1–K5 se construyeron y validaron con:
...
| **Informes (nacional)** | F3 — Oracle SISUD | En uso |
...
| **F3** | SISUD informes | **Informes** | — | Oracle | `SISUD.CSEP_INFORMES_VIEW` | `STG_ORA_CSEP_INFORMES` | `pl_stage_informes.hpl` | Hecho informe (`MI_FACT_INFORME_SUPERVISION`) |
...
**Dominio:** **Multas** alimenta `DF_MULTAS` / `MI_FACT_MULTA_COERCITIVA`; **Informes** alimenta `DF_INFORMES` / `MI_FACT_INFORME_SUPERVISION`. F2-ET y F2-DIC son apoyo al universo de multas, no generan hecho de informes.
...
|---|---|---|---|
| F3 | `oracle_sisud` | `SISUD` | `CSEP_INFORMES_VIEW` |
...
Tras el staging, `logica/dwh/` consume las lecturas H2 (`GS1`, `GS2`, `ORA`, `MYSQL`, `INFORMES`, `ETAPAS`):
...
Amarre multa ↔ informe (H9): expediente / `TXCUC` / `TXNUMEXP` / CUM+CAM → `MI_FACT_MULTA.ID_INFORME`.
