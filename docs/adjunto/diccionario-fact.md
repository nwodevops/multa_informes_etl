...
Celda vacía / `#N/A` en planilla → `NULL` en columnas de texto, fecha o monto. La fila **no se borra**. SISUD **no** pisa `N_RES_MC` ni `MONTO_UIT` / `MONTO_S` de la planilla.
...
| `ID_ADMINISTRADO`        | Administrado (`-1` si no hay)                  | `ADM`                                | —                                        |
...
- Si dice `— (SISUD si match)`: no está en Excel, pero **sí puede llenarse** desde SISUD. Sin match: `NULL` en CUM/medida/montos; `-1` en `ID_ESTADO_RESOLUCION`.
