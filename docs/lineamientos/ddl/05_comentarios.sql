...
COMMENT ON COLUMN MI_DIM_ORGANO_UNIDAD.SIGLA IS 'Sigla del órgano (ej. CMIN, CRES, OD-LAM).';
COMMENT ON COLUMN MI_DIM_ORGANO_UNIDAD.NOMBRE IS 'Nombre corto / igual a SIGLA si no hay alias.';
COMMENT ON COLUMN MI_DIM_ORGANO_UNIDAD.DESCRIPCION IS 'Nombre largo legible (ej. Minería, Residuos Sólidos) desde catálogo F2 CSEP.';
...
-- MI_DIM_OD
COMMENT ON TABLE MI_DIM_OD IS 'Dimensión territorio: oficina desconcentrada del Excel F1 (familia medidas administrativas). Distinta de MI_DIM_ORGANO_UNIDAD.';
COMMENT ON COLUMN MI_DIM_OD.ID_OD IS 'Clave surrogate; -1 = NO ESPECIFICADO (filas F2/F4/F5).';
COMMENT ON COLUMN MI_DIM_OD.COD_OD IS 'Slug de oficina (ICA, LAMBAYEQUE, PUNO, …) alineado al filename Excel.';
COMMENT ON COLUMN MI_DIM_OD.NOMBRE IS 'Nombre de la oficina o región.';
COMMENT ON COLUMN MI_DIM_OD.TIPO IS 'OD (departamento), ODES (oficina especial) o UNIDAD (CODE).';
COMMENT ON COLUMN MI_DIM_OD.ORDEN IS 'Orden de la lista institucional de oficinas; CODE comparte 9 con Huánuco.';
COMMENT ON COLUMN MI_DIM_OD.FECHA_ACTUALIZACION IS 'Fecha de última actualización del registro en el DW.';

-- MI_DIM_FUENTE_REGISTRO
COMMENT ON TABLE MI_DIM_FUENTE_REGISTRO IS 'Dimensión de universo/fuente de registro (F1 Sheets OD, F2 CSEP, F4 GAPP, F5 SISUD).';
COMMENT ON COLUMN MI_DIM_FUENTE_REGISTRO.ID_FUENTE IS 'Clave surrogate; -1 = NO ESPECIFICADO.';
COMMENT ON COLUMN MI_DIM_FUENTE_REGISTRO.CODIGO IS 'Código natural alineado a FUENTE_REGISTRO (OD_SHEETS, CAGR, GAPPS, SISUD_VW).';
COMMENT ON COLUMN MI_DIM_FUENTE_REGISTRO.NOMBRE IS 'Etiqueta corta legible de la fuente.';
COMMENT ON COLUMN MI_DIM_FUENTE_REGISTRO.FAMILIA_TDR IS 'Familia TDR: F1, F2, F4 o F5.';
COMMENT ON COLUMN MI_DIM_FUENTE_REGISTRO.DESCRIPCION IS 'Origen operativo y tabla STG asociada.';
COMMENT ON COLUMN MI_DIM_FUENTE_REGISTRO.FECHA_ACTUALIZACION IS 'Fecha de última actualización del registro en el DW.';
...
COMMENT ON COLUMN MI_FACT_MULTA_COERCITIVA.ID_OD IS 'FK a MI_DIM_OD (territorio F1); -1 si la fila no viene de Sheets OD.';
COMMENT ON COLUMN MI_FACT_MULTA_COERCITIVA.ID_FUENTE IS 'FK a MI_DIM_FUENTE_REGISTRO (universo F1–F5).';
COMMENT ON COLUMN MI_FACT_MULTA_COERCITIVA.ID_TIEMPO_FIRMA IS 'FK a MI_DIM_TIEMPO (AAAAMMDD) de F_FIRMA_RES_MC; -1 si no hay fecha.';
...
COMMENT ON COLUMN MI_DET_ETAPA_MC.ID_FUENTE IS 'FK a MI_DIM_FUENTE_REGISTRO; etapas F2 = CAGR.';
...
COMMENT ON TABLE MI_QA_AMARRE_DETALLE IS 'Claves que no amarran en cada puente H9 (SOLO_IZQ / SOLO_DER).';
COMMENT ON COLUMN MI_QA_AMARRE_DETALLE.LADO IS 'SOLO_IZQ o SOLO_DER respecto al puente.';
COMMENT ON COLUMN MI_QA_AMARRE_DETALLE.CLAVE IS 'Valor de clave natural sin match.';
COMMENT ON COLUMN MI_QA_AMARRE_DETALLE.MOTIVO IS 'Descripción corta del no-amarre.';
