...
-- MI_DIM_ESTADO
COMMENT ON TABLE MI_DIM_ESTADO IS 'Catálogo homologado de estados (resolución, multa, pago, informe, etapa, descargos).';
COMMENT ON COLUMN MI_DIM_ESTADO.ID_ESTADO IS 'Clave surrogate; -1 = NO ESPECIFICADO.';
COMMENT ON COLUMN MI_DIM_ESTADO.TIPO_ESTADO IS 'Dominio del estado: RESOLUCION, MULTA, PAGO, INFORME, ETAPA o DESCARGOS.';
...
COMMENT ON COLUMN MI_FACT_MULTA_COERCITIVA.NUMERO_EXPEDIENTE IS 'Expediente administrativo; clave de amarre H9 con informes.';
...
COMMENT ON COLUMN MI_FACT_MULTA_COERCITIVA.ID_INFORME IS 'FK opcional al informe de supervisión amarrado (H9); NULL si no hay cruce.';
...
COMMENT ON TABLE MI_FACT_INFORME_SUPERVISION IS 'Hecho: un informe de supervisión ambiental (fuente F3 SISUD).';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.ID_INFORME IS 'Clave surrogate del hecho informe.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.IDACTIVIDAD IS 'Identificador de actividad de supervisión en SISUD.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.TXCUC IS 'Código único de caso (CUC); clave de amarre H9.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.TXNUMEXP IS 'Número de expediente del informe en SISUD.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.TXINFORME IS 'Código o número del informe de supervisión.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.ID_ADMINISTRADO IS 'FK a MI_DIM_ADMINISTRADO; -1 = NO ESPECIFICADO.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.ID_ORGANO IS 'FK a MI_DIM_ORGANO_UNIDAD; -1 = NO ESPECIFICADO.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.ID_MATERIA IS 'FK a MI_DIM_MATERIA_SUBSECTOR; -1 = NO ESPECIFICADO.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.ID_ESTADO_INFORME IS 'FK a MI_DIM_ESTADO (TIPO_ESTADO=INFORME).';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.TIPO_SUPERVISION IS 'Tipo: ESPECIAL o REGULAR.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.FUENTE_PROGRAMACION IS 'Origen de programación: PLANEFA u OTRA.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.NIVEL_REVISION IS 'Nivel o instancia de revisión del informe.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.F_INICIO IS 'Fecha de inicio de la supervisión en campo.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.F_FIN IS 'Fecha de fin de la supervisión en campo.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.F_INFORME_ESPERADO IS 'Fecha límite esperada de entrega del informe.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.F_INFORME IS 'Fecha real de emisión del informe.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.F_REG_INFORME IS 'Fecha de registro formal del informe.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.DIAS_SUPERVISION IS 'Días entre F_INICIO y F_FIN.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.DIAS_ELAB_INFORME IS 'Días entre F_FIN y F_INFORME.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.FLAG_INFORME_OPORTUNO IS '1 = informe entregado a tiempo; 0 = fuera de plazo.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.FLAG_DERIVADO IS '1 = informe derivado de otro procedimiento; 0 = no.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.FUENTE_REGISTRO IS 'Fuente origen; valor fijo SISUD_INF.';
COMMENT ON COLUMN MI_FACT_INFORME_SUPERVISION.FECHA_CARGA IS 'Fecha y hora de carga de la fila en el DW.';
