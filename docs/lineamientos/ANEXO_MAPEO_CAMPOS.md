...
| `ID_INFORME` | resuelto por amarre | `NUMERO_EXPEDIENTE` ↔ F3 `TXCUC`/`TXNUMEXP` | FK a `FACT_INFORME_SUPERVISION`; `NULL`/`-1` si no amarra (H9) |
| `ID_ADMINISTRADO` | resuelto vía informe amarrado (F3 `IDADMINISTRADO`) | F5 `ADMINISTRADO` (si no hay amarre) | lookup en `DIM_ADMINISTRADO`; `-1` si no resuelve |
| `ID_ORGANO` | F2 `COORD` | sigla final de `NUMERO_EXPEDIENTE` (ej. `...-DSIS-CRES`) | lookup en `DIM_ORGANO_UNIDAD`; `-1` si no resuelve |
| `ID_MATERIA` | vía informe amarrado (F3 `TXSUBSECTOR_UND`) | — | lookup en `DIM_MATERIA_SUBSECTOR`; `-1` si no resuelve |
| `ID_ESTADO_RESOLUCION` | F5 `ESTADO_RESOLUCION` | — | homologar contra `DIM_ESTADO` (`TIPO_ESTADO='RESOLUCION'`) |
| `ID_ESTADO_MULTA` | F1/F2 `ESTADO_MC` | F5 `ESTADO_MULTA`; F4 `FG_ESTADOMULTA` (conciliar) | homologar contra `DIM_ESTADO` (`TIPO_ESTADO='MULTA'`) |
| `ID_ESTADO_PAGO` | F2 `ESTADO_PAGO_MC` | — | homologar contra `DIM_ESTADO` (`TIPO_ESTADO='PAGO'`) |
| `ID_UIT` | resuelto por año | `YEAR(F_FIRMA_RES_MC)` → `YEAR(FECHA_EMISION F5)` → `YEAR(FN_MC)` | lookup en `DIM_PARAMETRO_UIT` |
...
| `VALOR_UIT_APLICADO` | `DIM_PARAMETRO_UIT.VALOR_UIT` del año resuelto en `ID_UIT` | — | lookup |
...
| `ID_ADMINISTRADO` | `IDADMINISTRADO` / `TXADMINISTRADO` | lookup/alta en `DIM_ADMINISTRADO` |
| `ID_ORGANO` | `TXCOORDINACION` | lookup/alta en `DIM_ORGANO_UNIDAD` |
| `ID_MATERIA` | `TXSUBSECTOR_UND` | lookup/alta en `DIM_MATERIA_SUBSECTOR` |
| `ID_ESTADO_INFORME` | `TXESTADO` (o `TXPRY_ESTADO` si aplica al ciclo del informe) | homologar contra `DIM_ESTADO` (`TIPO_ESTADO='INFORME'`) |
...
|---|---|---|
| `ID_MC` | resuelto por amarre `COD_PROY_MC` | lookup contra `FACT_MULTA_COERCITIVA.COD_PROY_MC`; `NULL` si aún no existe el hecho padre |
...
| `DIAS_ELABORACION` | `T_ELAB_MC` | validar/recalcular con `DIM_TIEMPO.ES_DIA_HABIL` si se requiere precisión |
...
## 5. Tabla `DQ_HALLAZGO` — qué la alimenta
...
cambia de estructura; solo cambia el origen de `DIM_TIEMPO.ES_FERIADO`, `DIM_ORGANO_UNIDAD`/
`DIM_MATERIA_SUBSECTOR` (si trae UBIGEO) y `DIM_PARAMETRO_UIT` (si trae la UIT oficial vigente).
