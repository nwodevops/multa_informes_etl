...
>
> **Diseño vigente:** fact de negocio `DW_M_FACT_MULTA_COERCITIVA` = planillas **sede central (10) + OD (31) + GAPPS (F4)**; SISUD lookup CUM/CAM solo sobre F1∪F2. **No hay base CSEP.** Attrs operativos en el hecho: `JEFE`, `UF`, `N_PROY_MC`, `ETA_REG_*`, `RESULT_PROY_MC`, `ESTADO_*_TXT`.
...
| F4 | MySQL gapps | `vw_multas_app.sql` → `STG_MYSQL_MULTAS` | molde Excel F2 + CUM/CAM |
...
| `GAPPS` | Fila procedente de F4 (app MySQL; grano `NU_IDMC`) |
