...
5. Carga Oracle/MySQL vía `stage_if_configured.sh` si hay credenciales reales.
...
WHERE owner = 'APP'
  AND table_name IN (
    'FACT_MULTA_COERCITIVA', 'FACT_INFORME_SUPERVISION',
    'INDICADOR_RESULTADO', 'DQ_HALLAZGO'
  );

SELECT COD_INDICADOR, COUNT(*) FROM APP.INDICADOR_RESULTADO
